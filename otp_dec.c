 ...
 * Name: otp_dec.c
 * Description: This file is serving as the client for the decrypting part of the project4
 *   assignment. This file will grab the cipher and key from the folder and send it 
 *   to the server to be decrypted.
 * ********************************************************************************************/
...
char * process_file(char filename[])
{
	...
	while(buffer[charctr] != EOF)
	{
		...
		{
			if(buffer[charctr] != ' ' && buffer[charctr] != 32 && buffer[charctr] != 10)
			{
