 ...
 * Name: otp_dec_d.c
 * Description: This file servers as the decryption side server part for project 4. This file
 *   will accept the ciphertext and the key from the client and dencrypt it. Then send 
 *   the dncryption back to the client. 
 * ********************************************************************************************/
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
 ...
 * Description: This function will calculate the ciphertext in order to decrypt
 * ********************************************************************************************/
...
int main(int argc, char * argv[])
{
	...
	while(1)
	{
		...
		{		
			...
			if (strcmp(buffer, "otp_dec here!\0") == 0)
			{
			...
			// sleep to allow time to recv all bytes
			sleep(2);	
			...
			charsRead = send(establishedConnectionFD, "ok", 3, 0); // Send success back
			// sleep to allow to recv all bytes
			sleep(2);
			memset(key, '\0', 70000);
